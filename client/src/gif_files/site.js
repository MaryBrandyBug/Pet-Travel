...
$(function(){
   var fpl=function(){	
      $('.photo2_list_items').each(function(){	  
         ...
         var inHeight=$(this).parent().parent().data('photo2_list-height');
         var inSingleRow=$(this).parent().parent().data('photo2_list-row');        //Отображать в один ряд

         var inSingleRowMobile=$(this).parent().parent().data('photo2_list-row-xs');     //Отображать в один ряд в мобильных
         var inHeightMobile=$(this).parent().parent().data('photo2_list-height-xs');

         var paramHeight=inHeight?1*inHeight:(MOB?1000:320);
         ...
         if (paramSingleRow){
            ...
            $(this).parent().parent().css('display', 'block'); //Не работает collagePlus если wrapper it-scroll-slider display:flex
         }       
...
$.c4_dom(function(){
   $(this).find('.it-autocomplete-js').each(function(){
      ...
      var oldWord=''; //Блокировка повторных отправок слова на сервер

		if (!url) return;
		var getWord=function(){
         var tval=$t.val().split(' ');
         word=(tval.length>1?tval[tval.length-2]+' ':'')+tval[tval.length-1];
         return word;
      }
      ...
      $t.on('keyup',function(e){
         ...
         if ((e.keyCode>=48 && e.keyCode<=57) || (e.keyCode>=65 && e.keyCode<=290)){
            if (!e.metaKey && !e.altKey && !e.ctrlKey && !e.shiftKey && e.key!="Meta")
            if (letter)
               if (letter.length==1)
                  if (letter.match("^[A-Za-z0-9А-Яа-я]$")){
                     word=getWord();
                  }
         }else{
            word='';
         ...
         clearTimeout(to);
         var to=setTimeout(()=>{
            if (word && oldWord!=word){
					oldWord=word;
               $.get(url,{q:word},function(d){
                  $container.html('');
                  $items=[];
                  for (a in d){
